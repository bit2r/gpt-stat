...
x <- create_image("draw Korean ink painting style landscape image, green tone, minimalism.")
...
# 2. 로고 ------------------------------------------------------------
## 2.1. 소스 이미지
stat_bg <- magick::image_read("images/raw_image.png")
...
stat_gg <- ggplot() +
  ...
  annotate("richtext", x=0.50, y = 0.04, family = ft_github,
           size = 11, angle = 30,
           colour = txt, hjust = 0,
           label = glue("<span style='font-family:fa-brands; color:{txt}'>&#xf09b;&nbsp;</span> bit2r/gpt-stat"),
